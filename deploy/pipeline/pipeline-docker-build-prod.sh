...
cp pipeline-deploy-kube-prod.sh $ARCHIVE_DIR/

#Update deployment.yml with image name
if [ -f deployment-prod.yml ]; then
    echo "UPDATING DEPLOYMENT MANIFEST:"
    sed -i "s~^\([[:blank:]]*\)image:.*$~\1image: ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/${IMAGE_NAME}:${BUILD_NUMBER}~" deployment-prod.yml
    cat deployment-prod.yml
    if [ ! -f $ARCHIVE_DIR/deployment-prod.yml ]; then # no need to copy if working in ./ already    
        cp deployment-prod.yml $ARCHIVE_DIR/
    fi
else 
    echo -e "${red}Kubernetes deployment file 'deployment-prod.yml' not found at the repository root${no_color}"
    exit 1
fi      
